...
class STCEncoder(nn.Module):
    def __init__(self, in_channels, embed_dim=128, num_layers=2):
        super().__init__()
        self.embed_dim = embed_dim

        # Spatial Feature Extraction with SAME padding (Preserve H, W)
        self.spatial_conv = nn.Sequential(
            nn.Conv3d(in_channels, embed_dim, kernel_size=(1, 3, 3), stride=(1, 1, 1), padding=(0, 1, 1)),
            nn.SiLU(),
            nn.Conv3d(embed_dim, embed_dim, kernel_size=(1, 3, 3), stride=(1, 1, 1), padding=(0, 1, 1)),
            nn.SiLU()
        )
...
class Unet3D(nn.Module):
    def __init__(
        ...
        temporal_attn_scm = lambda dim: EinopsToAndFrom('b c f h w', 'b (h w) f c',
                                                    Attention(dim, heads=attn_heads, dim_head=attn_dim_head,
                                                              rotary_emb=rotary_emb))
        temporal_attn_mask = lambda dim: EinopsToAndFrom('b c f h w', 'b (h w) f c',
                                                    Attention(dim, heads=attn_heads, dim_head=attn_dim_head,
                                                              rotary_emb=rotary_emb))
        ...
        self.stc_encoder = STCEncoder(in_channels=2, embed_dim=init_dim)
        self.stc_encoder_2 = STCEncoder(in_channels=init_dim, embed_dim=init_dim)
        #self.stc_encoder_scm = STCEncoder(in_channels=2, embed_dim=init_dim)
        self.init_conv = nn.Conv3d(channels, init_dim, (1, init_kernel_size, init_kernel_size),
                                   padding=(0, init_padding, init_padding))

        self.modality_enc = nn.Conv3d(1, init_dim, (1, init_kernel_size, init_kernel_size),
                                   padding=(0, init_padding, init_padding))
        self.modality_enc_mask = nn.Conv3d(2, init_dim, (1, init_kernel_size, init_kernel_size),
                                   padding=(0, init_padding, init_padding))

        self.init_temporal_attn = Residual(PreNorm(init_dim, temporal_attn(init_dim)))
        self.init_temporal_attn_mask = Residual(PreNorm(init_dim, temporal_attn(init_dim)))
        self.init_temporal_attn_scm = Residual(PreNorm(init_dim, temporal_attn(init_dim)))

        self.init_spatial_attention = Residual(PreNorm(init_dim,SpatialLinearAttention(dim=init_dim, heads=4, dim_head=32)))
        self.norma_layer = LayerNorm(2)
        ...
        self.mid_spatial_attn_scm = Residual(PreNorm(mid_dim, spatial_attn))
        self.mid_temporal_attn = Residual(PreNorm(mid_dim, temporal_attn(mid_dim)))
        #self.mid_temporal_attn_mask = Residual(PreNorm(mid_dim, temporal_attn(mid_dim)))
        self.mid_temporal_attn_scm = Residual(PreNorm(mid_dim, temporal_attn(mid_dim)))
...
class GaussianDiffusion(nn.Module):
    ...
    def p_mean_variance(self, x, t, fea, scm, mask, clip_denoised: bool, cond=None, cond_scale=1.):
        fea = fea.unsqueeze(dim=2).repeat(1, 1, x.size(2), 1, 1)
        x_recon = self.predict_start_from_noise(x, t=t, noise=self.denoise_fn.forward_with_cond_scale(x,
                                                                                                      t,
                                                                                                      scm=scm,
                                                                                                      mask=mask,
                                                                                                      cond=cond,
    ...
    def p_sample(self, x, t, fea, scm, mask, cond=None, cond_scale=1., clip_denoised=True):
        b, *_, device = *x.shape, x.device
        model_mean, _, model_log_variance = self.p_mean_variance(x=x, t=t, fea=fea, scm=scm, mask=mask,
    ...
    @torch.inference_mode()
    def p_sample_loop(self, fea, shape, scm, mask, dense, cond=None, cond_scale=1.):
        ...
        for i in tqdm(reversed(range(0, self.num_timesteps)), desc='sampling loop time step', total=self.num_timesteps):
            img = self.p_sample(img, torch.full((b,), i, device=device, dtype=torch.long), fea, scm, mask, cond=cond,
                                cond_scale=cond_scale)
    ...
    def sample(self, fea, scm, mask, dense, cond=None, cond_scale=1., batch_size=16):
        ...
        return sample_fn(fea, (batch_size, channels, num_frames, image_size, image_size), scm, mask, dense, cond=cond,
                         cond_scale=cond_scale)

    # add by nhm
    @torch.no_grad()
    def ddim_sample(self, fea, shape, mask, cond=None, cond_scale=1., clip_denoised=True):
    ...
    def p_losses(self, x_start, t, fea, scm, mask, cond=None, noise=None, clip_denoised=True, **kwargs):
        ...
        if is_list_str(cond):
            ...
            cond = cond.to(device)
        pred_noise = self.denoise_fn.forward(x_noisy, t, scm= scm, mask=mask, cond=cond,
    ...
    def forward(self, x, fea, scm, mask, text, *args, **kwargs):
